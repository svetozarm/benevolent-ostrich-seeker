...
do
    ...
    runspec --action=build $BENCH_NAME --config=llvmemtrack
    cp $SPEC_INSTALL/benchspec/CPU2006/$BENCH_NAME/build/build*0000/map_variables.json $RESULT_DIR
    scp $RESULT_DIR/map_variables.json smiucin@sumi.cs.sfu.ca:/cs/systems/home/smiucin/memwatch/
    read -p "Variable map copied, start the node server on sumi, and spark and hit return"
